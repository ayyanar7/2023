package com.ag.bta.main.wavelayout;
...
public class WaveLayout extends FrameLayout {
    ...
    private Canvas canvasForWaveMask;
    ...
    public WaveLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        try {
            WaveAngle = a.getInteger(R.styleable.WaveLayout_wave_angle, DEFAULT_ANGLE);
            WaveAnimationDuration = a.getInteger(R.styleable.WaveLayout_animation_duration, DEFAULT_ANIMATION_DURATION);
            WaveColor = a.getColor(R.styleable.WaveLayout_wave_color, getColor(R.color.textPrimaryGrid));
            autoStart = a.getBoolean(R.styleable.WaveLayout_auto_start, false);
            maskWidth = a.getFloat(R.styleable.WaveLayout_mask_width, 0.5F);
            gradientCenterColorWidth = a.getFloat(R.styleable.WaveLayout_gradient_center_width, 0.1F);
            isAnimationReversed = a.getBoolean(R.styleable.WaveLayout_reverse_animation, false);
        } finally {
            a.recycle();
        }
    ...
    public void startWaveAnimation() {
        ...
        Animator animator = getWaveAnimation();
        animator.start();
    ...
    public void setWaveAnimationDuration(int durationMillis) {
        this.WaveAnimationDuration = durationMillis;
        resetIfStarted();
    }
    ...
    /**
     * Set the angle of the Wave effect in clockwise direction in degrees.
    ...
    public void setWaveAngle(int angle) {
        if (angle < MIN_ANGLE_VALUE || MAX_ANGLE_VALUE < angle) {
            throw new IllegalArgumentException(String.format("WaveAngle value must be between %d and %d",
                    MIN_ANGLE_VALUE,
                    MAX_ANGLE_VALUE));
        }
        this.WaveAngle = angle;
        resetIfStarted();
    }

    /**
     * Sets the width of the Wave line to a value higher than 0 to less or equal to 1.
     * 1 means the width of the Wave line is equal to half of the width of the WaveLayout.
     * The default value is 0.5.
     *
     * @param maskWidth The width of the Wave line.
     */
     ...
     * 0.99 means that the whole Wave line will have this color with a little transparent edges.
    ...
    private void dispatchDrawWave(Canvas canvas) {
        super.dispatchDraw(canvas);
        ...
        if (canvasForWaveMask == null) {
            canvasForWaveMask = new Canvas(localMaskBitmap);
        }

        canvasForWaveMask.drawColor(Color.TRANSPARENT, PorterDuff.Mode.CLEAR);

        canvasForWaveMask.save();
        canvasForWaveMask.translate(-maskOffsetX, 0);

        super.dispatchDraw(canvasForWaveMask);

        canvasForWaveMask.restore();
    ...
    private void drawWave(Canvas destinationCanvas) {
        createWavePaint();
    ...
    private void createWavePaint() {
        if (gradientTexturePaint != null) {
            return;
        }

        final int edgeColor = reduceColorAlphaValueToZero(WaveColor);
        final float WaveLineWidth = getWidth() / 2 * maskWidth;
        final float yPosition = (0 <= WaveAngle) ? getHeight() : 0;

        LinearGradient gradient = new LinearGradient(
                0, yPosition,
                (float) Math.cos(Math.toRadians(WaveAngle)) * WaveLineWidth,
                yPosition + (float) Math.sin(Math.toRadians(WaveAngle)) * WaveLineWidth,
                new int[]{edgeColor, WaveColor, WaveColor, edgeColor},
    ...
    private Animator getWaveAnimation() {
        if (maskAnimator != null) {
            return maskAnimator;
        }
        ...
        final int WaveBitmapWidth = maskRect.width();
        final int WaveAnimationFullLength = animationToX - animationFromX;

        maskAnimator = isAnimationReversed ? ValueAnimator.ofInt(WaveAnimationFullLength, 0)
                : ValueAnimator.ofInt(0, WaveAnimationFullLength);
        maskAnimator.setDuration(WaveAnimationDuration);
        ...
        maskAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
            	...
            	maskOffsetX = animationFromX + (int) animation.getAnimatedFraction()*100;

                if (maskOffsetX + WaveBitmapWidth >= 0) {
                    invalidate();
                }
            }
        });
    ...
    private int calculateMaskWidth() {
        final double WaveLineBottomWidth = (getWidth() / 2 * maskWidth) / Math.cos(Math.toRadians(Math.abs(WaveAngle)));
        final double WaveLineRemainingTopWidth = getHeight() * Math.tan(Math.toRadians(Math.abs(WaveAngle)));

        return (int) (WaveLineBottomWidth + WaveLineRemainingTopWidth);
    }
    ...
    /**
     * in WaveLayout is used ComposeShader, which contains bug in android 4.1.1 with layer hardware acceleration
     * @see <a href="https://stackoverflow.com/questions/12445583/issue-with-composeshader-on-android-4-1-1">
     * StackOverflow</a>
     */
