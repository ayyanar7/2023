package com.ag.bta.main.fragments.navdrawer.about.views;
...
import com.ag.bta.main.fragments.navdrawer.about.AboutBuilder;
import com.ag.bta.main.fragments.navdrawer.about.model.Item;
import com.ag.bta.main.fragments.navdrawer.about.utils.RippleUtil;
import com.ag.bta.main.fragments.navdrawer.about.utils.VisibleUtil;
...
private FrameLayout cvHolder;
 //  private CardView cvHolder;
    ...
    AboutBuilder obundle;
    private void init(AboutBuilder bundle,int layout) {
        ...
        setLayoutParams(lp);
        layoutInflater.inflate(layout, holder);

         //layoutInflater.inflate(R.layout.about_xab_about_layout_card, holder);
    }

    private void bind() {
        cvHolder = (FrameLayout)findViewById(R.id.card_holder);
    ...
    public void buildAdmin(@NonNull AboutBuilder bundle) {
        init(bundle, R.layout.about_xab_about_layout_card);
        loadAll(bundle);
           }
    
    public void buildHome(@NonNull AboutBuilder bundle) {
        init(bundle, R.layout.about_homepage_xab_layout_card);
        loadAll(bundle);
           }

    
    private void loadAll(@NonNull AboutBuilder bundle) {
    	 bind();
         ...
         if (bundle.getBackgroundColor() != 0)
             cvHolder.setBackgroundColor(bundle.getBackgroundColor());
    ...
    private int getCardColor() {
         ...
         return obundle.getBackgroundColor();
    }

    private int getIconColor() {
        if (iconColor == 0)
            iconColor = isDarker() ? Color.WHITE : Color.TRANSPARENT;// getNameColor();
    ...
    private void loadLinks(AboutBuilder bundle) {
        for (Item item : bundle.getLinks()) {
            View v = addItem(vLinks, R.layout.about_xab_each_link, item);
    ...
    private void animate(final View v) {
        ...
        new Handler().postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                v.startAnimation(AnimationUtils.loadAnimation(getContext(), R.anim.about_expand_in));
            }
        }, animationDelay);
    }
    ...
    private void loadActions(AboutBuilder bundle) {
        for (Item item : bundle.getActions()) {
            addItem(vActions, R.layout.about_xab_each_action, item);
        }
    }
    ...
    private View addItem(ViewGroup holder, int layout, Item item) {
        ...
        ivIcon.setBackgroundColor(getIconColor());
    ...
    private void setupCard(AboutBuilder bundle) {
        if (!bundle.isShowAsCard()) {
        	cvHolder.setElevation(0);
        	cvHolder.setClipToPadding(false);
